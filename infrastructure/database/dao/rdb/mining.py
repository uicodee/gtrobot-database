...
from infrastructure.database.models import (
    MiningUsers, MiningTasks, MiningReferrals, MiningHistory,
    MiningSubscriptionOrders, UserMiningHistory, SubscribersMining, BudgetHistory, UserTasks, UserEarnings,
    UserWithdraws, SubscriptionOrders, UserPromoCodes, BalanceHistory, HoldingBalances, WithdrawTransactions,
    UserSubscriptions, UserExchanges
)
...
class MiningDAO(BaseDAO[MiningUsers]):
    ...
    async def get_mining_user_data(self, user_id: int) -> Dict[str, str]:
        ...
        result = await self.session.execute(
            select(
                ...
                func.coalesce(SubscribersMining.subscription, 'standard').label('subscription'),
            ...
            .outerjoin(
                MiningReferrals,
                MiningUsers.user_id == MiningReferrals.referrer_user_id
            )
        ...
        dict_keys = [
            'user_id', 'referral_code', 'balance', 'daily_mining_balance',
            'subscription', 'crypto_currency', 'registration_date', 'referral_count'
        ]
    ...
    async def get_mining_user_referrals(self, user_id: int) -> List[Dict[str, str]]:
        ...
        dict_keys = ['referral_user_id', 'referral_date', 'referral_user_name', 'referral_user_profile_photo']
    ...
    async def get_mining_active_tasks(self) -> List[Dict[str, str]]:
        ...
        dict_keys = ['task_id', 'task_text', 'task_link', 'task_group_id', 'is_required']
        return [dict(zip(dict_keys, data)) for data in result.fetchall()]
    ...
    async def get_mining_user_completed_task_ids(self, user_id: int) -> List[int]:
        result = await self.session.execute(
            select(MiningTasks.task_id)
            .where(MiningTasks.user_id == user_id)
        )
        return [row[0] for row in result.fetchall()]

    async def get_last_user_mining_data(self, user_id: int, mining_type: str) -> Dict[str, str]:
        result = await self.session.execute(
            select(
    ...
    async def get_mining_end_message_not_sent(self, mining_type: str, mining_date: int) -> List[Dict[str, str]]:
        result = await self.session.execute(
            select(
            ...
            .outerjoin(
                SubscribersMining, SubscribersMining.user_id == UserMiningHistory.user_id
            )
            .where(
    ...
    async def get_users_mining_data_by_id(self, mining_id: int) -> List[Dict[str, str]]:
        ...
        dict_keys = [
            'mining_id', 'user_id', 'mining_date', 'mining_type',
            'subscription', 'crypto_currency', 'balance', 'referral_count'
        ]
        return [dict(zip(dict_keys, row)) for row in result.fetchall()]

    async def get_users_mining_data_by_mining_type(self, mining_type: str) -> List[Dict[str, str]]:
        result = await self.session.execute(
            select(
                UserMiningHistory.id,
                UserMiningHistory.mining_id,
    ...
    async def get_current_mining_budget(self, budget_type: str = 'regular') -> Dict[str, str]:
        result = await self.session.execute(
            select(
                BudgetHistory.id,
    ...
    async def get_count_mining_users(self) -> int:
        result = await self.session.execute(
            select(func.count(MiningUsers.user_id))
        )
        return result.scalar() if result else 0

    async def get_mining_user_earning(self, user_id: int, mining_id: int) -> float:
        result = await self.session.execute(
            select(UserEarnings.earning)
            .where(
                UserEarnings.user_id == user_id,
                UserEarnings.mining_id == mining_id
            )
        )
    ...
    async def get_mining_user_earnings(self, mining_id: int) -> float:
        result = await self.session.execute(
            select(UserEarnings.earning)
            .where(UserEarnings.mining_id == mining_id)
        )
    ...
    async def get_top_earning_accounts(self, ticker_prices: Dict[str, float], top_count: int = 20) -> List[Dict[str, str]]:
        dict_keys = ['user_id', 'daily_mining_balance', 'balance', 'crypto_currency', 'registration_date']
        users_data = []
    ...
    async def get_active_subscribers_mining_users(self, end_date: int) -> List[Dict[str, str]]:
        result = await self.session.execute(
            select(
        ...
        dict_keys = ['id', 'user_id', 'start_date', 'end_date', 'subscription', 'end_message_sent']
        return [dict(zip(dict_keys, row)) for row in result.fetchall()]

    async def get_subscribers_mining_end_message_not_sent(self, end_date: int) -> List[Dict[str, str]]:
        result = await self.session.execute(
            select(
                SubscribersMining.id,
                SubscribersMining.user_id,
    ...
    async def get_mining_user_withdraws_not_sent(self) -> List[Dict[str, str]]:
        ...
        dict_keys = [
            'withdraw_id', 'user_id', 'withdraw_source_currency', 'withdraw_source_amount',
            'withdraw_currency', 'withdraw_amount', 'withdraw_address', 'withdraw_date',
            'is_sent', 'is_checked', 'is_confirmed'
        ]
        return [dict(zip(dict_keys, row)) for row in result.fetchall()]
    ...
    async def get_users_count_for_statistics(self, mining_type: str, offset: int = 4) -> Dict[str, int]:
        ...
        result = await self.session.execute(
            select(
                func.coalesce(
                    select(func.count(UserMiningHistory.user_id.distinct()))
                    .where(UserMiningHistory.mining_id == current_mining_subquery),
                    0
                ).label('current_mining_users_count'),
                func.coalesce(
                    select(func.count(UserMiningHistory.user_id.distinct()))
                    .where(UserMiningHistory.mining_id == previous_mining_subquery),
                    0
    ...
    async def get_subscription_order(self, query_id: int) -> Dict[str, str]:
        ...
        dict_keys = ['query_id', 'user_id', 'currency', 'total_amount', 'invoice_payload', 'shipping_option_id', 'date']
        data = result.fetchone()
        return dict(zip(dict_keys, data)) if data else {}
    ...
    async def promo_code_data(self, promo_code: str) -> Dict[str, str]:
        ...
        dict_keys = ['id', 'user_id', 'promo_code', 'promo_code_sum', 'is_used', 'promo_code_date']
        data = result.fetchone()
        return dict(zip(dict_keys, data)) if data else {}
    ...
    async def get_users_without_profile_photo(self) -> List[int]:
        result = await self.session.execute(
            select(MiningUsers.user_id)
            .where(MiningUsers.user_profile_photo == None)
        )
        return [row[0] for row in result.fetchall()]

    async def get_inactive_balances(self, ticker_prices: Dict[str, float], balance_date: int,
                                    min_usdt_balance: float = 1) -> List[Dict[str, str]]:
        dict_keys = ['user_id', 'source_balance', 'usdt_balance', 'crypto_currency', 'balance_date']
        users_data = []
    ...
    async def get_holding_balance(self, user_id: int, max_holding_date: int, is_withdrawn: bool = False) -> Dict[str, str]:
        result = await self.session.execute(
            select(
                HoldingBalances.id,
    ...
    async def get_user_inactive_balance(self, user_id: int, crypto_currency: str, ticker_prices: Dict[str, float],
                                        min_usdt_balance: float = 1) -> Dict[str, str]:
        ...
        result = await self.session.execute(
            select(
                ...
                case(
                    ...
                    (and_(UserWithdraws.is_checked == True, UserWithdraws.is_confirmed == True), 1),
                    (and_(UserWithdraws.is_checked == True, UserWithdraws.is_confirmed == False), -1),
                    else_=None
                ).label('status')
    ...
    async def get_withdraw_transaction_id(self, withdraw_id: int) -> Optional[int]:
        result = await self.session.execute(
            select(WithdrawTransactions.transaction_id)
            .where(WithdrawTransactions.withdraw_id == withdraw_id)
        )
        return result.scalar()

    async def get_users_count_in_current_time(self, mining_type: str, from_date: int = None,
                                              is_unique: bool = False) -> int:
    ...
    async def get_users_earnings(self, from_date: int = 0) -> float:
        result = await self.session.execute(
            select(func.sum(UserEarnings.earning))
            .where(
                UserEarnings.earning < 10,
                UserEarnings.earning_date > from_date
            )
        )
        return result.scalar() or 0.0

    async def update_user_mining_crypto_currency(self, user_id: int, balance: float, crypto_currency: str):
    ...
    async def update_user_mining_balance(self, user_id: int, user_balance: float, crypto_currency: str):
        await self.session.execute(
            update(MiningUsers)
            .where(MiningUsers.user_id == user_id)
    ...
    async def update_user_daily_mining_balances(self, data: List[Tuple[float, int]]):
        for daily_balance, user_id in data:
            await self.session.execute(
                update(MiningUsers)
                .where(MiningUsers.user_id == user_id)
                .values(daily_mining_balance=MiningUsers.daily_mining_balance + daily_balance)
            )
        await self.session.commit()

    async def update_mining_history_is_checked(self, mining_id: int, is_checked: bool = True):
        await self.session.execute(
    ...
    async def update_mining_user_subscription(self, user_id: int, subscription: str = 'standard'):
        await self.session.execute(
            update(MiningUsers)
            .where(MiningUsers.user_id == user_id)
    ...
    async def update_mining_user_withdraw_sent_status(self, withdraw_id: int, is_sent: bool):
        await self.session.execute(
    ...
    async def update_mining_user_withdraw_confirmed_status(self, withdraw_id: int, is_confirmed: bool):
        await self.session.execute(
            update(UserWithdraws)
            .where(UserWithdraws.id == withdraw_id)
    ...
    async def update_mining_user_withdraw_checked_status(self, withdraw_id: int, is_checked: bool):
        await self.session.execute(
            update(UserWithdraws)
            .where(UserWithdraws.id == withdraw_id)
            .values(is_checked=is_checked)
    ...
    async def update_subscribers_mining_end_message_sent(self, subscribers_mining_id: int, end_message_sent: bool):
        await self.session.execute(
    ...
    async def update_user_profile_photo(self, user_id: int, user_profile_photo: Optional[str] = None):
        await self.session.execute(
            update(MiningUsers)
            .where(MiningUsers.user_id == user_id)
    ...
    async def update_holding_balance_is_withdrawn(self, balance_hold_id: int, is_withdrawn: bool = True):
        await self.session.execute(
    ...
    async def update_user_promo_code_status(self, promo_code_id: int, is_used: bool = True):
        await self.session.execute(
    ...
    async def update_user_subscription(self, user_id: int, subscription: str, end_subscription_date: Optional[int] = None):
        if end_subscription_date:
            await self.session.execute(
                update(UserSubscriptions)
                .where(UserSubscriptions.user_id == user_id)
                .values(subscription=subscription, end_subscription_date=end_subscription_date)
            )
        else:
            await self.session.execute(
                update(UserSubscriptions)
    ...
    async def set_mining_user(self, user_id: int, referral_code: str, user_profile_photo: Optional[str] = None,
                              crypto_currency: str = 'btc', registration_date: Optional[int] = None):
    ...
    async def set_mining_referral_user(self, referrer_user_id: int, referral_user_id: int, referral_user_name: str,
    ...
    async def set_user_mining_history(self, mining_id: int, user_id: int, mining_type: str, mining_date: Optional[int] = None):
    ...
    async def set_user_mining_tasks(self, user_id: int, task_id: int, completion_date: Optional[int] = None):
    ...
    async def set_mining_cycle(self, start_date: int, end_date: int, mining_type: str, is_checked: bool = False):
        result = await self.session.execute(
            insert(MiningHistory).values(
    ...
    async def set_mining_users_earnings(self, data: List[Tuple[int, int, float, int]]):
        await self.session.execute(
            insert(UserEarnings),
            [{"user_id": user_id, "mining_id": mining_id, "earning": earning, "earning_date": earning_date} for user_id, mining_id, earning, earning_date in data]
        )
        await self.session.commit()
    ...
    async def set_mining_task(self, task_text: str, task_link: str, task_group_id: Optional[int] = None, is_required: bool = False):
        await self.session.execute(
    ...
    async def set_mining_subscribers_mining(self, user_id: int, start_date: int, end_date: int, subscription: str):
        await self.session.execute(
    ...
    async def set_mining_user_exchange(self, user_id: int, from_currency: str, to_currency: str, from_balance: float,
                                       to_balance: float, exchange_date: Optional[int] = None):
    ...
    async def set_mining_user_withdraw(self, user_id: int, source_currency: str, source_amount: float,
                                       withdraw_currency: str, withdraw_amount: float, withdraw_address: str,
                                       withdraw_date: Optional[int] = None, is_sent: bool = False,
                                       is_confirmed: bool = False, holding_id: Optional[int] = None):
    ...
    async def set_mining_withdraw_transactions(self, withdraw_id: int, transaction_id: str, transaction_date: Optional[int] = None):
    ...
    async def set_mining_subscription_order(self, query_id: int, user_id: int, currency: str, total_amount: float,
                                            invoice_payload: str, shipping_option_id: Optional[str] = None, date: Optional[int] = None):
    ...
    async def create_user_subscription(self, user_id: int, subscription: str, end_subscription_date: int,
                                       order_id: Optional[int] = None, token_id: Optional[int] = None, purchase_date: Optional[int] = None):
    ...
    async def create_holding_balance(self, user_id: int, source_balance: float, usdt_balance: float, holding_date: Optional[int] = None):
    ...
    async def set_mining_user_balance_histories(self, data: List[Tuple[int, float, str, int]]):
        await self.session.execute(
            insert(BalanceHistory),
            [{"user_id": user_id, "balance": balance, "crypto_currency": crypto_currency, "balance_date": balance_date} for user_id, balance, crypto_currency, balance_date in data]
        )
        await self.session.commit()
    ...
    async def del_mining_user_task(self, user_id: int, task_id: int) -> None:
        await self.session.execute(
            delete(UserTasks)
            .where(UserTasks.user_id == user_id, UserTasks.task_id == task_id)
        )
        await self.session.commit()
