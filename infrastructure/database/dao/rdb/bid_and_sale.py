...
class BidAndSaleDAO(BaseDAO[BidAndSaleUsers]):
    ...
    async def get_active_users_with_symbol_id(
        self, symbol_id: int, tx_value: int
    ) -> List[int]:
        result = await self.session.execute(
            select(BidAndSaleUsers.user_id)
            .join(
                BidAndSaleUserSymbols,
                BidAndSaleUserSymbols.user_id == BidAndSaleUsers.user_id,
            )
            .where(
    ...
    async def get_active_users(self) -> List[int]:
        result = await self.session.execute(
            select(BidAndSaleUsers.user_id).where(BidAndSaleUsers.is_active == True)
        )
        return [row[0] for row in result.fetchall()]
    ...
    async def get_user_symbols(self, user_id: int) -> List[str]:
        result = await self.session.execute(
            ...
            .join(
                BidAndSaleUserSymbols,
                BidAndSaleUserSymbols.symbol_id == BidAndSaleSymbols.id,
            )
    ...
    async def get_user_exists(self, user_id: int) -> bool:
        result = await self.session.execute(
            select(BidAndSaleUsers).where(BidAndSaleUsers.user_id == user_id)
        )
        return result.fetchone() is not None

    async def get_user_active_status(self, user_id: int) -> bool:
        result = await self.session.execute(
            select(BidAndSaleUsers.is_active).where(BidAndSaleUsers.user_id == user_id)
        )
    ...
    async def add_user(self, user_id: int, is_active: bool = True):
        await self.session.execute(
            insert(BidAndSaleUsers).values(user_id=user_id, is_active=is_active)
        )
        await self.session.commit()
    ...
    async def add_user_symbols(self, data: list):
        await self.session.execute(
            ...
            [
                {"user_id": user_id, "symbol_id": symbol_id, "tx_value": tx_value}
                for user_id, symbol_id, tx_value in data
            ],
        )
        await self.session.commit()

    async def add_symbol(self, symbol: str):
        await self.session.execute(insert(BidAndSaleSymbols).values(name=symbol))
        await self.session.commit()
