...
from infrastructure.database.models import (
    P2PUsers, P2PPairs, P2PPairsExchange
)
...
class P2PDAO(BaseDAO[P2PUsers]):
    ...
    async def get_all_p2p_autodelete_messages(self) -> List[Dict[str, int]]:
        result = await self.session.execute(
            select(P2PUsers.user_id, P2PUsers.message_id, P2PUsers.current_time)
        )
        dict_keys = ['user_id', 'message_id', 'current_time']
        return [dict(zip(dict_keys, row)) for row in result.fetchall()]

    async def get_new_p2p_pair_data(self) -> List[Dict[str, str]]:
        result = await self.session.execute(
            select(
            ...
            ).where(P2PPairs.is_post == False)
        )
        dict_keys = ['id', 'profit', 'asset_buy', 'price_buy', 'payment_buy',
                     'asset_sell', 'price_sell', 'payment_sell', 'exchange', 'currency']
        return [dict(zip(dict_keys, [str(itm) for itm in row])) for row in result.fetchall()]

    async def get_new_p2p_pair_exchanges_data(self) -> List[Dict[str, str]]:
        result = await self.session.execute(
            select(
            ...
            ).where(P2PPairsExchange.is_post == False)
        )
        dict_keys = ['id', 'profit', 'asset', 'price_buy', 'exchange_buy', 'price_sell', 'exchange_sell']
        return [dict(zip(dict_keys, [str(itm) for itm in row])) for row in result.fetchall()]

    async def get_active_p2p_user_data(self) -> List[Dict[str, str]]:
        result = await self.session.execute(
            select(
                P2PUsers.user_id, P2PUsers.profit_percent, P2PUsers.exchanges
            ).where((P2PUsers.is_active == True) | (P2PUsers.is_active_exchanges == True))
        )
        dict_keys = ['user_id', 'profit_percent', 'exchanges']
        return [dict(zip(dict_keys, [str(itm) for itm in row])) for row in result.fetchall()]

    async def get_active_p2p_user_exchanges_data(self) -> List[Dict[str, str]]:
        result = await self.session.execute(
            select(
                P2PUsers.user_id, P2PUsers.profit_percent, P2PUsers.exchanges
            ).where(P2PUsers.is_active_exchanges == True)
        )
        dict_keys = ['user_id', 'profit_percent', 'exchanges']
        return [dict(zip(dict_keys, [str(itm) for itm in row])) for row in result.fetchall()]
    ...
    async def p2p_pair_exists(self, asset_buy: str, payment_buy: str, asset_sell: str, payment_sell: str, exchange: str) -> bool:
        result = await self.session.execute(
            select(P2PPairs)
            .where(
