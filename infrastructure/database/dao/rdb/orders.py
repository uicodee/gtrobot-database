...
from infrastructure.database.models import (
    UzumPayment, UzumAppPayment, PaymentNumbers, Purchases,
)
...
class OrdersDAO(BaseDAO[UzumPayment]):
    ...
    async def get_uzum_payment_order_data(self, order_id: str) -> Dict[str, Optional[str]]:
        result = await self.session.execute(
            select(
                UzumPayment.id, UzumPayment.user_id, UzumPayment.first_name,
                UzumPayment.username, UzumPayment.last_operation_id,
                UzumPayment.order_id, UzumPayment.currency_code,
                UzumPayment.provider, UzumPayment.tariff_plan,
                UzumPayment.total_amount, UzumPayment.promo_code,
    ...
    async def update_uzum_payment_is_completed(self, order_id: str, status: int = 1) -> None:
    ...
    async def update_uzum_app_payment_is_completed(self, transaction_id: str, status: int = 1) -> None:
        await self.session.execute(
    ...
    async def update_uzum_payment_last_operation_id(self, order_id: str, operation_id: str) -> None:
        await self.session.execute(
            update(UzumPayment)
            .where(UzumPayment.order_id == order_id)
    ...
    async def update_uzum_transaction_id(self, order_id: str, transaction_id: str) -> None:
        await self.session.execute(
            update(UzumPayment)
            .where(UzumPayment.order_id == order_id)
    ...
    async def set_uzum_payment(self, user_id: int, first_name: str, username: str, operation_id: str,
                               order_id: str, currency_code: str, provider: str, tariff_plan: str,
                               total_amount: int, promo_code: Optional[str] = None) -> int:
        result = await self.session.execute(
            insert(UzumPayment).values(
                user_id=user_id,
    ...
    async def set_uzum_app_payment(self, phone_number: str, plan: str,
                                   total_amount: int, transaction_id: Optional[str] = None,
                                   order_opening_time: Optional[int] = None, is_completed: int = 0) -> int:
        result = await self.session.execute(
            insert(UzumAppPayment).values(
    ...
    async def set_new_purchase(self, user_id: int, provider: str, tariff_plan: str, total_amount: int,
                               purchase_time: int, name: Optional[str] = None, username: Optional[str] = None,
                               phone_number: Optional[str] = None, promo_code: Optional[str] = None,
                               referral_user: Optional[str] = None, is_purchase: int = 1) -> int:
        result = await self.session.execute(
