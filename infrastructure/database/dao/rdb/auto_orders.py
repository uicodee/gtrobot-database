...
from infrastructure.database.models import Positions, AutoOrderUsersOrders, LeaderboardUsers, LeaderboardOrders, \
    ClosedLeaderboardOrders


class AutoOrderUsersDAO(BaseDAO):
    def __init__(self, session: AsyncSession):
        super().__init__(session)

    async def del_auto_order_position(self, position_id: int) -> None:
        query = delete(Positions).where(Positions.position_id == position_id)
        await self.session.execute(query)

    async def del_auto_order_user_order(self, user_id: int, position_id: int) -> None:
        query = delete(AutoOrderUsersOrders).where(AutoOrderUsersOrders.user_id == user_id,
                                                   AutoOrderUsersOrders.position_id == position_id)
        await self.session.execute(query)

    async def del_auto_order_leaderboard_user(self, encrypted_uid: str, creator_user_id: int) -> None:
    ...
    async def del_closed_leaderboard_order(self, order_id: int) -> None:
        query = delete(ClosedLeaderboardOrders).where(ClosedLeaderboardOrders.id == order_id)
        await self.session.execute(query)
